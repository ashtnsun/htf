...
Changed on September 9, 2026 (Session 12a): Ashton asked for the Get involved graphic to be the interactive globe from the Partners section of the Nonprofits page, with the pins hoverable. `layout/involved/GlobeGraphic` wraps `globe/PartnerGlobe` in the frame (no pointer tilt, the globe's own glow) and is the new default, in the bundle (the SVG globe with pins; the three.js scene is a lazy chunk fetched when the section is near the viewport, as on /nonprofits). Its pins are the partner locations from the project files, handed down by `layout/ContactCta` with a `label`: the country, the last comma-separated part of each project's `location` (`PartnerLocation.country`). In the scene a labelled pin has an invisible hit disc; hovering it lights the pin mint, freezes the globe (`SpinController.hover`) and shows a hairline rectangle with the country name and a leader line down to the pin, placed from the frame loop through `globe/LabelAnchor` (a DOM element over the canvas, never React state per frame); leaving the pin resumes the spin at once. Pins on the far side are not hoverable (a facing test, since the raycaster sees through the occluding sphere). The Nonprofits globe is unchanged: its pins carry no label. Terminal, Chat and Badge stay in the menu.
...
## 22. Amendments — the dino process scene, reviewed (September 8, 2026)

Ashton's review of §21, applied the same day (Session 11h). **Hats are worn**: every hat's brim covers the head's top row and is drawn the head's width, so the deerstalker, the hard hat (now a dome with a ridge and a visor, not a cone) and the party hat sit on the head. **The team is the same dino**: Match shows the footer T-rex three times, the lead facing two mirrored teammates, one in square glasses and one in a backwards cap; the other species are gone. **Deliver has one dinosaur**: the partner is gone, the T-rex holds the gift out at its side toward the step text and the confetti falls across the whole stage; the arm stays joined to the body and to the box in every frame (it gains a cell as the box is pushed forward). **The dissolve runs on its own clock**: the scroll only picks the step (nearest step centre, with a small hysteresis band at each midpoint), and the scene then dissolves to it cell by cell over about half a second and holds, so each picture is on screen for the whole of its step and no scroll position lands inside a transition; reduced motion cuts instead. Nothing attached to the body is ever translated: the magnifying glass lifts one row rather than circling, and each moving prop is drawn as whole frames that start beside the hand. Later the same day (September 9): the teammates are two smaller plain copies of the T-rex (a hand-reduced 14 × 16 map on the same grid), one pink and one blue; `--pink #FF3D9E` and `--blue #3388FF` join the palette as decoration-only tokens built like the greens, and the glasses and the cap are gone. Later on September 9: every step is a viewport tall from lg and 60% of one on phones, so a scroll gesture cannot fly past a step, and the confetti falls only over the dino and the gift.

## 23. Amendments — review pass 5, Google Forms and the parked portal (September 9, 2026)

Ashton's fourteen-item review after §20's globe (Session 13). **Applications run through a Google Form this cycle and the in-house portal is parked**: `/apply` is a static page that embeds the form (`content/site.ts` → `season.applyFormUrl`, framed with `embedded=true`, with a link to open it on its own; a visible TODO and the Instagram button until the link is pasted in; "closed for now" out of season), every Apply CTA already points there, and `season.applyMode` / `NEXT_PUBLIC_APPLY_MODE` are gone. The portal of §5 and §12–§16 (sign-in, the multi-step form, the exec dashboard) and the contact / intake forms of §3 are finished but moved, history intact, into `parked/src/…` (excluded from the build, type-check, lint and Prettier) with `parked/README.md` listing what is there, its environment variables and the restore steps; `supabase/` stays at the repo root. **Nonprofit requests and contact go by email**: /nonprofits "Start a project" is an "Email us" button with the address (the intake form is gone, and the copy in `content/process.ts`, `content/faq.ts` and `content/nonprofits.ts` says email rather than form), and /contact is the hero plus three tiles (email, LinkedIn, Instagram). `content/privacy.mdx` covers email and the Google Form (Google as a processor) instead. The site now needs no environment variable (`.env.example`, `docs/DEPLOY.md`).

The rest of the review: the testimonial pause button sits on the label's line (home) and on the "Partners say" line (nonprofits); the awards table has three equal columns so the award is in the table's middle (home and About); the Get involved globe no longer bobs (the CSS translate on the WebGL canvas was the flicker) and is exactly centred beside the copy; project-card arrows are white at rest and nudge right on hover; About loses "Who we are" (`content/about.ts` keeps only the mission); the section bar marks the current section with green text, no underline; the Students and Nonprofits banners have no CTA; the roles intro sticks in a viewport-tall box below the header and the bar with its copy centred, and the per-role Apply button is gone; the recruitment timeline drops the deadline note and the 01–06 labels, marks the current step (`current: true` in `content/recruitment.ts`, Callouts today) with the green node, ping and a "Now" tag, and its rail runs off the right edge of the screen past Project kickoff; the How it works cards on /nonprofits share their rows through a subgrid so titles, descriptions and "Your part" line up; and the dotted map behind the nonprofit quotes is gone. PROGRESS.md (Session 13) has the measurements and the TODOs (the form link and the club email are Ashton's).

## 24. Amendments — review pass 6 (September 9, 2026)

Ashton's eight-item review after §23 (Session 14). **Home:** the Who we serve panels open with a pixel picture each (a stack of books for students, a hand holding out a heart for nonprofits), drawn like the process scene on the dinosaur's cell grid; the How it works list starts half a viewport under its heading and ends half a viewport after Deliver, with a short bottom padding before Impact (the gaps roughly halved); the Photo hero has no button; a hovered pin on the Get involved globe names the state for a partner in the US and the country elsewhere. **Project pages** (revising §3's spec): a plain title block instead of the banner, the write-up with the team and the stack in a sticky right column, no jump links and no "More projects" rail; the gallery and the closing CTA stay. **Students:** the roles intro is top-aligned and sticks just below the section bar; the timeline's current step keeps its green node and loses the "Now" tag. **/apply** (revising §23): one centred column and no banner: a short heading with the deadline, the embedded form, and an FAQ under it (`audience: "apply"` in `content/faq.ts`) that absorbs "What happens next"; the roles list is gone. PROGRESS.md (Session 14) has the measurements.
